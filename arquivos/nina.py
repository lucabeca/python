...
def limpador_bucal(palavroes):
    ...
    for palavra in palavroes:
        if palavra == True:
            palavrones.append(True)
        else:
            nao_palavroes.append(False)
    if len(palavrones) > len(nao_palavroes):
        return True
    return False
...
def espirros_gastadores(quantidade_que_espirrei):
    ...
    custo = 0
    for espirro in range(1, quantidade_que_espirrei + 1):
        custo += (espirro + (espirro * (0.5 * (espirro - 1))))
    return custo
...
def gerente(vendas):
    ...
    abaixo_da_media_anual = []
    media_anual = round(sum(vendas) / len(vendas), 2)
    meses = {
        0: "jan",
        1: "fev",
        2: "mar",
    ...
    for mes, valor in enumerate(vendas):
        if valor < media_anual:
            abaixo_da_media_anual.append(meses[mes])
    return len(abaixo_da_media_anual), abaixo_da_media_anual
